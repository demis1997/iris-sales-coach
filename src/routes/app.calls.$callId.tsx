  ...
  return (
    <>
      ...
      <Panel className="mt-4">
        <PanelHeader
          title="Post-call AI package"
          subtitle="Summary · CRM · sentiment · risk · next steps — approve before write-back"
          action={
            <Link to="/crm/deal-risk" className="text-xs text-primary hover:underline">
              Open deal risk →
            </Link>
          }
        />
        <div className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-3 text-xs">
          {[
            ["Summary", "15pt close-rate gap confirmed; open to 2-team pilot if ROI clear."],
            ["Action items", "Lock calendar hold · Send ROI · Do not discount yet"],
            ["CRM notes", "Budget mid-six figures · Champion = Marcus · Competitor = Gong"],
            ["Follow-up email", "Draft ready — ROI math + Monday/Tuesday pilot slots"],
            ["Sentiment", "72 positive overall · dip at price objection"],
            ["Buying intent", "81 · rising after ROI reframe"],
            ["Objections", "Price comparison vs competitor"],
            ["Competitors", "Gong — battlecard used"],
            ["Compliance", "Passed · consent stated"],
            ["Risk score", "34 medium · timeline missing"],
            ["Win probability", "78% (was 72%)"],
            ["Personality", "Analytical · risk-aware · time-pressed"],
          ].map(([k, v]) => (
            <div key={k} className="rounded-xl border border-border bg-secondary/25 p-3">
              <p className="font-semibold text-foreground">{k}</p>
              <p className="mt-1 text-muted-foreground leading-relaxed">{v}</p>
            </div>
          ))}
        </div>
